...
// Template object for new image & window dimensions
var newDimObj = {
    width: 0,
    height: 0,
    x_center: 0,
...
/**
 * Set the global variables within this module. This is a function instead
 * of an instantiator because it may have to be reset multiple times
...
/**
 * Round the new calculated values before returning them. This is 
 * becauseElectron cannot resize windows to a decimal-based value.
 * @method roundNewValues
 * @param  {[type]}       nd [description]
 * @return {[type]}          [description]
 */
function roundNewValues(nd) {
    return {
        x_center: Math.round(nd.x_center),
        y_center: Math.round(nd.y_center),
...
function centerImage(imageDimensions) {
    ...
    return roundNewValues(newDimensions);
}


/**
 * [maximizeDimensions description]
 * @method maximizeDimensions
 * @return {[type]}           [description]
 */
function maximizeDimensions(imageDimensions) {
    var newDimensions = {
        x_center: 0,
        y_center: 0,
    ...
    // Ensure nothing breaks the bounds
...
// maybe a 'scale' function that scales to the window
// a 'fill' function that fills to the max dimensions
