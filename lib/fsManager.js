    ...
    if (filepath !== this.folder) {
        ...
        fs.readdir(filepath, (err, items) => {
            if (!err) {
                ...
                //
                // items.sort((a, b) => {
                //     var aMatch = a.match(/^[^0-9]+[0-9]+/);
                //     var bMatch = b.match(/^[^0-9]+[0-9]+/);
                //
                //     if (aMatch && bMatch) {
                //         sortDiff = aMatch[0].match(/[0-9]+/)[0] - bMatch[0].match(/[0-9]+/)[0];
                ...
                /*items.sort((a, b) => {
                    var aNumStart = a.match(/^[0-9]+/);
                    var bNumStart = b.match(/^[0-9]+/);
                    var aNumEnd = a.match(/[0-9]+\..*$/);
                    var bNumEnd = b.match(/[0-9]+\..*$/);

                    // console.log(aNumStart);
                    // console.log(bNumStart);
                    // console.log(aNumEnd);
                    // console.log(bNumEnd);

                    // Try to sort numerically by the numbers at the start of the filename
                    if (aNumStart && bNumStart) {
                        num_diff = aNumStart[0] - bNumStart[0];
                    ...
                    } else if (aNumEnd && bNumEnd) {
                        num_diff = aNumEnd[0].match(/[0-9]+/)[0] - bNumEnd[0].match(/[0-9]+/)[0];
